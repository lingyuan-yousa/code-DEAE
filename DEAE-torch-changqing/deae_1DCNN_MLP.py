...
def train_model(encoder, x_train, y_train, x_unlab, x_test, y_test, parameters, p_m, K, beta, T1=20, T2=100):
    ...
    y_pseudo = torch.zeros(len(x_unlab), dtype=torch.long)  # Assume class starts from 0
    ...
    for epoch in range(parameters['iterations']):
        ...
        with torch.no_grad():
            x_encoded = encoder(x_train)  # Encode the labeled data
        ...
        for idx in range(K):
            ...
            with torch.no_grad():
                xu_temp = encoder(xu_temp)  # Encode the unlabeled data

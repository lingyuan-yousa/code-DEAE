...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query):
        # For using MultiheadAttention, the shape of query needs to be (L, N, E)
        # L is the sequence length, N is the batch size, and E is the number of features (embedding dimension)
        # If the shape of the input query is (N, E), unsqueeze(0) is needed to add the sequence length dimension, e.g., (1, N, E)
...
class DADE_Self(nn.Module):
    def __init__(self, dim, alpha, num_heads=4, hid1=64, hid2=64):
        ...
        self.mask_estimator = nn.Sequential(
            nn.Linear(dim, hid1),  # The first hidden layer, with dim as the input dimension and hid1 as the output dimension of this layer
            nn.LeakyReLU(),  # ReLU activation function for the first hidden layer
            self.attention,  # Attention layer
            nn.Linear(hid1, hid2),  # The second hidden layer, with hid1 as the input dimension and hid2 as the output dimension of this layer
            nn.LeakyReLU(),  # ReLU activation function for the second hidden layer
            nn.Linear(hid2, dim),  # Output layer, with hid2 as the input dimension and dim as the output dimension, keeping the same output size as the original model

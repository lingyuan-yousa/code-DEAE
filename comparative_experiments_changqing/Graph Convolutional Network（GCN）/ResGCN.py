...
# Create a dictionary mapping unique lithology names to numerical categories
unique_liths = data['LITH'].unique()
lith_to_category = {lith: category for category, lith in enumerate(unique_liths)}

# Replace Chinese labels in the LITH column with numerical categories using the replace method
data['LITH'] = data['LITH'].replace(lith_to_category)

# Select rows containing the keyword 'Le' as the indices for the test set
test_idx = data[data['Well_Name'].str.contains('Le')].index

# Select rows not containing the keyword 'Le' as the indices for the training set
...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv3 = GCNConv(hid + graph_data.num_features, len(torch.unique(graph_data.y)))  # Use the number of unique values in y as the number of output classes

...
def train_model(encoder, x_train, y_train, x_unlab, x_test, y_test, parameters, p_m, K, beta, T1=20, T2=100):
    ...
    y_pseudo = torch.zeros(len(x_unlab), dtype=torch.long)  # Assume class labels start from 0
    ...
    for epoch in range(parameters['iterations']):
        ...
        for idx in range(K):
            with torch.no_grad():
                xu_temp = encoder(x_unlab)  # Encode unlabeled data
    ...
    # In PyTorch, usually disable gradient calculation during prediction
    with torch.no_grad():
        y_test_hat_logit, y_test_hat = predictor(x_test)

    # Convert y_test_hat to NumPy array if needed
...
def compute_accuracy(predictor, x_test, y_test):
    ...
    with torch.no_grad():  # Disable gradient calculation
        ...
        correct = (y_test_pred == y_test).sum().item()  # Compute the number of correct predictions
        total = y_test.size(0)  # Total number in the test set
